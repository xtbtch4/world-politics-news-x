...
6. Создайте второй секрет `OPENAI_API_KEY` и вставьте API-ключ OpenAI.

Токены и ключи нельзя добавлять непосредственно в файлы репозитория. При отсутствии `OPENAI_API_KEY` публикация останавливается, чтобы канал не получал низкокачественный машинный перевод.
